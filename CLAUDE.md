...
> the merge; subsequent work extended the ledger, which is now contiguous **0001–0030**
> (verified against the `decisions` collection on 2026-06-28; next free **0031**).
> Ciclo 0026–0030 (2026-06-28): 0026 auto-bootstrap de root local; 0027 versionamiento
...
> constructora + indexador maestro + hook de seed.
...
- Architectural changes get an ADR via the `record_decision` MCP tool (next free id;
  currently 0031). Keep ADR ids contiguous and never reuse one. The number is the
